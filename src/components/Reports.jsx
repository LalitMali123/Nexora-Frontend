...
const Reports = () => {
    ...
    const downloadPDF = () => {
        ...
        if (reportType === 'monthly') {
            ...
            autoTable(doc, {
                ...
                columnStyles: {
                    0: { cellWidth: 30 },
                    1: { cellWidth: 35, textColor: [16, 185, 129] },
                    2: { cellWidth: 35, textColor: [239, 68, 68] },
                    3: { cellWidth: 35 },
                    4: { cellWidth: 30 }
                }
            });
            ...
            autoTable(doc, {
                ...
                columnStyles: {
                    0: { cellWidth: 40 },
                    1: { cellWidth: 35, textColor: [239, 68, 68] },
                    2: { cellWidth: 30 },
                    3: { cellWidth: 35 },
                    4: { cellWidth: 30 }
                }
    ...
    const downloadReport = () => {
        ...
        const exportFileDefaultName = `expense_report_${reportType}_${new Date().toISOString().slice(0, 10)}.json`;
    ...
    const exportToCSV = () => {
        ...
        if (reportType === 'monthly') {
            ...
            csvData = [['Category', 'Total Spent', 'Number of Transactions', 'Average per Transaction', 'Percentage']];
            const categoryData = getCategoryReport();
            const totalExpenseAmount = categoryData.reduce((sum, [_, data]) => sum + data.amount, 0);
            ...
            <div className="container">
                ...
                <div className="summary-stats">
                    ...
                    <div className="stat-box">
                        ...
                        <p className="expense-text">₹{totalExpenses.toFixed(2)}</p>
                    </div>
                    <div className="stat-box">
                        <h3>Net Savings</h3>
                        <p className={totalIncome - totalExpenses >= 0 ? 'income-text' : 'expense-text'}>
                ...
                <div className="report-controls">
                    ...
                    <div className="download-buttons">
                        ...
                        <button onClick={exportToCSV} className="btn-csv">
                            📊 Download CSV
                        </button>
                    </div>
                </div>
                                    ...
                                    <tbody>
                                        {monthlyReport.map(([month, data]) => (
                                            <tr key={month}>
                                                ...
                                                <td className={data.savings >= 0 ? 'income' : 'expense'}>
                                                    ₹{data.savings.toFixed(2)}
                                                </td>
                                                <td>{data.count}</td>
                                            </tr>
                                ...
                                <table>
                                    ...
                                    <tbody>
                                        {categoryReport.map(([category, data]) => (
                                            <tr key={category}>
                                                <td>
                                                    <span className="category-icon">{data.icon}</span> {category}
                                                </td>
