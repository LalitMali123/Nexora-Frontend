...
const Reports = () => {
    ...
    const downloadPDF = () => {
        ...
        if (reportType === 'monthly') {
            ...
            const tableData = monthlyData.map(([month, data]) => [month, `₹${data.income.toFixed(2)}`, `₹${data.expense.toFixed(2)}`, `₹${data.savings.toFixed(2)}`, data.count.toString()]);
            autoTable(doc, { startY: startY, head: [['Month', 'Income', 'Expenses', 'Savings', 'Transactions']], body: tableData, theme: 'striped', headStyles: { fillColor: [102, 126, 234], textColor: [255, 255, 255] }, styles: { fontSize: 8, cellPadding: 3 } });
        } else {
            ...
            const tableData = categoryData.map(([category, data]) => [`${data.icon} ${category}`, `₹${data.amount.toFixed(2)}`, data.count.toString(), `₹${(data.amount / data.count).toFixed(2)}`, `${((data.amount / totalExpenseAmount) * 100).toFixed(1)}%`]);
            autoTable(doc, { startY: startY, head: [['Category', 'Total Spent', 'Transactions', 'Average', 'Percentage']], body: tableData, theme: 'striped', headStyles: { fillColor: [102, 126, 234], textColor: [255, 255, 255] }, styles: { fontSize: 8, cellPadding: 3 } });
        }
    ...
    const downloadCSV = () => {
        ...
        if (reportType === 'monthly') {
            ...
            getMonthlyReport().forEach(([month, data]) => { csvData.push([month, data.income, data.expense, data.savings, data.count]); });
        } else {
            csvData = [['Category', 'Total Spent', 'Transactions', 'Average', 'Percentage']];
            const categoryData = getCategoryReport();
            const totalExpenseAmount = categoryData.reduce((sum, [_, data]) => sum + data.amount, 0);
            categoryData.forEach(([category, data]) => { csvData.push([category, data.amount, data.count, (data.amount / data.count).toFixed(2), `${((data.amount / totalExpenseAmount) * 100).toFixed(1)}%`]); });
        }
